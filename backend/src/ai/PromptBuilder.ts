...
export class PromptBuilder {
	...
	buildPlanningPrompt(input: PlanningContext): string {
		...
		return [
			...
			"- Use remember_place when the player says this location is their home, base, mine, farm, village, portal, or asks Yuri to remember this place.",
            "- For remember_place, placeName must be short and lowercase when possible.",
            "- Use go_to_place when the player asks Yuri to go to, lead them to, or take them to a remembered place.",
            "- For go_to_place, only use a placeName that appears in relevant memories or is clearly requested by the player.",
            "- Never invent coordinates. The backend resolves remembered places into coordinates.",
            "- If no relevant place memory is available, reply that Yuri does not remember that place yet and use no action.",
            "- Use stop_action when the player asks Yuri to stop, cancel, wait, leave it, stop hunting, stop mining, or stop the current task.",
            "- stop_action does not need parameters.",
			"",
			"Recent conversation:",
			recentConversation,
			"",
